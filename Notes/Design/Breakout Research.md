...
- **Constant positive reinforcement.** Block Blast pops "Good! / Amazing!" and rainbow sparkles on every clear. We gave particles only; score changed silently in a corner. **Done (issue #26):** floating "+N" popups rise from every killed block (value × multiplier, so combos visibly pay) and a big "COMBO x…" callout fires at each well entered.
