...
build:                         build:  (same gate re-runs)
  ...
  (test step where tests exist) push-image:  (only on push to main)
  ← blocks merge if red          docker build → push ghcr.io/alon-shviki/<image>:latest
```
...
| Repo | Build project | Image |
|------|---------------|-------|
| `game-portal` | `portal-auth/PortalAuth.csproj` | `ghcr.io/alon-shviki/portal-auth:latest` |
| `Bullet-Heaven` | `BulletHeaven.Client` (+ tests) | `ghcr.io/alon-shviki/bh-client:latest` |
| `orbit-break` | `OrbitBreak.Client` (+ tests) | `ghcr.io/alon-shviki/orbit-break-client:latest` |
...
jobs:
  build:
    ...
    steps:
      ...
      - name: Test          # omit if the repo has no test project
...
- **No gate gets skipped.** Lint/test/build red → fix the code, don't disable the check or delete the test.
...
agent reproduces locally (dotnet build/test) → fixes root cause → pushes → CI re-runs
```
...
## Optimization (only if a pipeline gets slow)

Current pipelines are well under 10 min, so leave them. If one grows: add NuGet caching (`actions/setup-dotnet` with `cache: true`, or `actions/cache` on `~/.nuget/packages`), and split independent jobs to run in parallel. Larger/self-hosted runners are a last resort, not a first reach.
